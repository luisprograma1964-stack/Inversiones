...
def ejecutar_control():
    print("==================================================")
    print("[+] INICIANDO CONTROL DE CALIDAD Y CONFIGURACIÓN")
    print("==================================================")
    ...
    print("[*] Descargando hojas: MAESTRO_ACTIVOS e HISTORICO_VALORES...")
    try:
    ...
    print(f"[OK] Cargados {len(df_maestro)} activos en el Maestro.")
    print(f"[OK] Cargados {len(df_historico)} registros en el Histórico.")
    ...
    print("\n--- 1. CONTROL DE INTEGRIDAD REFERENCIAL ---")
    ...
    if activos_sin_historial:
        print(f"[!] ADVERTENCIA: {len(activos_sin_historial)} activos ACTIVOS en el maestro no tienen ningún registro histórico:")
        print(f"    {sorted(list(activos_sin_historial))}")
    else:
        print("[OK] Todos los activos activos en el maestro tienen al menos un registro en el histórico.")
    ...
    if huerfanos:
        print(f"[!] ALERTA: {len(huerfanos)} tickers en HISTORICO_VALORES no existen en MAESTRO_ACTIVOS (huérfanos):")
        print(f"    {sorted(list(huerfanos))}")
    else:
        print("[OK] No hay tickers huérfanos en el histórico.")
    ...
    if inactivos_con_datos:
        print(f"[*] NOTA: {len(inactivos_con_datos)} activos INACTIVOS en el maestro tienen registros históricos:")
        print(f"    {sorted(list(inactivos_con_datos))}")

    # 4. CONTROL DE FORMATO Y VALORES NULOS
    print("\n--- 2. CONTROL DE VALORES NULOS Y FORMATOS ---")
    
    # Fechas no válidas
    fechas_nulas = df_historico['FECHA'].isnull().sum()
    if fechas_nulas > 0:
        print(f"[!] ALERTA: Hay {fechas_nulas} registros con fechas nulas o en formato inválido.")
    else:
        print("[OK] No hay fechas nulas o inválidas.")
    ...
    for col in ['PRECIO_CIERRE', 'VOLUMEN', 'MAXIMO_DIA', 'MINIMO_DIA']:
        if col in df_historico.columns:
            ...
            if nulos > 0:
                print(f"[!] ALERTA: Columna {col} tiene {nulos} valores nulos o no numéricos.")
            else:
                print(f"[OK] Columna {col} no tiene valores nulos.")

    # 5. CONTROL DE COHERENCIA DE PRECIOS Y VOLUMEN
    print("\n--- 3. CONTROL DE COHERENCIA DE PRECIOS Y VOLUMEN ---")
    ...
    if not precios_invalidos.empty:
        print(f"[!] ALERTA: Hay {len(precios_invalidos)} registros con precio de cierre <= 0:")
        print(precios_invalidos[['TICKER_ID', 'FECHA', 'PRECIO_CIERRE']].head(10).to_string())
    else:
        print("[OK] Todos los precios de cierre son mayores a cero.")
    ...
    if not max_menor_min.empty:
        print(f"[!] ALERTA: Hay {len(max_menor_min)} registros donde MAXIMO_DIA < MINIMO_DIA:")
        print(max_menor_min[['TICKER_ID', 'FECHA', 'MAXIMO_DIA', 'MINIMO_DIA']].head(10).to_string())
    else:
        print("[OK] No hay registros donde el máximo del día sea menor al mínimo.")
    ...
    if not cierre_fuera_filtrado.empty:
        print(f"[!] ADVERTENCIA: Hay {len(cierre_fuera_filtrado)} registros donde el precio de cierre está fuera del rango [Mínimo, Máximo] del día:")
        print(cierre_fuera_filtrado[['TICKER_ID', 'FECHA', 'MINIMO_DIA', 'PRECIO_CIERRE', 'MAXIMO_DIA']].head(10).to_string())
    else:
        print("[OK] Los precios de cierre se encuentran dentro del rango [Mínimo, Máximo] diario.")

    # 6. ANÁLISIS DE ANOMALÍAS DE PRECIO (VARIACIONES EXTREMAS)
    print("\n--- 4. VARIACIONES EXTREMAS Y ANOMALÍAS (POSIBLES ERRORES / SPLITS) ---")
    ...
    if not variaciones_extremas.empty:
        print(f"[!] DETECTADO: Hay {len(variaciones_extremas)} variaciones de precio diarias mayores al {VARIACION_LIMITE * 100}%:")
        ...
        for idx, row in variaciones_extremas.sort_values(by='RET_DIA', key=abs, ascending=False).head(15).iterrows():
            ...
            print(f"    - {row['TICKER_ID']} el {fecha_str}: Var: {pct:+.2f}% | Cierre anterior y actual bajo sospecha. Valor: {row['PRECIO_CIERRE']:.2f}")
    else:
        print(f"[OK] No se detectaron variaciones diarias extremas (> {VARIACION_LIMITE*100}%).")

    # 7. ANÁLISIS DE RECIENCIA Y BRECHAS (GAPS)
    print("\n--- 5. RECIENCIA Y BRECHAS DE ACTUALIZACIÓN ---")
    ...
    if tickers_desactualizados:
        print(f"[!] ALERTA: {len(tickers_desactualizados)} activos activos no registran actualizaciones recientes (últimos 7 días):")
        for ticker, ult_fecha, dias in sorted(tickers_desactualizados, key=lambda x: x[2], reverse=True)[:15]:
            print(f"    - {ticker}: última actualización el {ult_fecha} ({dias} días sin datos)")
    else:
        print("[OK] Todos los activos activos tienen datos actualizados al menos en los últimos 7 días.")

    if brechas_grandes:
        print(f"\n[!] BRECHAS: Se detectaron {len(brechas_grandes)} saltos de fechas inusuales (> 4 días) entre registros:")
        for ticker, f_desde, f_hasta, dias in sorted(brechas_grandes, key=lambda x: x[3], reverse=True)[:15]:
            print(f"    - {ticker}: brecha de {dias} días sin datos entre {f_desde} y {f_hasta}")
    else:
        print("[OK] No se detectaron brechas de días inusuales (> 4 días) entre registros consecutivos.")

    print("\n==================================================")
    print("[+] CONTROL DE CALIDAD Y CONFIGURACIÓN FINALIZADO")
    print("==================================================")
    return True
